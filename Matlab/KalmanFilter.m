...
u = ones(1,length(t));       % Input signal or Control vector

y = zeros(1,length(t));      % Initial Ideal Measurement output

y_hat = zeros(1,length(t));  % Initial Estimated output

noise_v_pct = 5;            % Measurement Noise in Percent

xactual = x;
for i=1:numIters  
    y(i) = C*xactual;                 % Ideal Measurement output
    xactual = A*xactual + B*u(i);
end
